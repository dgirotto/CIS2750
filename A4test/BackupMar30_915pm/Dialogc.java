...
public class Dialogc extends JFrame implements ActionListener{
   ...
   public static void GenCode(){
      ...
      try{
         ...
         /* create the FieldEdit interface */
         fw = new FileWriter(noExtension + "FieldEdit.java");
         output = new BufferedWriter(fw);

         output.write("import java.io.*;\n");
         output.write("public interface " + noExtension + "FieldEdit{\n");       
        
         //getLValName("fields"); 
         for(x=0;x<fieldCtr;x++){
            fieldName = getLValName("fields");
            output.write("public String getDC" + fieldName + "();\n");
            output.write("public void setDC" + fieldName + "(String toSet);\n");
         }         
         output.write("public void appendToStatusArea(String msg);\n}\n");
         output.close();
         /* interface complete */

         /* create the GUI which extends JFrame and implements the FieldEdit interface */ 
         fw = new FileWriter(noExtension + ".java");
         output = new BufferedWriter(fw);
         ...
         output.write("public class " + noExtension + " extends JFrame implements " + noExtension + "FieldEdit{\n");
         ...
         for(x=0;x<btnCtr;x++){
            buttonName = getLValName("buttons");
            output.write("myBtns["+x+"] = new JButton(\""+buttonName+"\");\n");
            output.write("addActionListener(new "+ buttonName +"(this));\n"); /* attach the external actionlisteners */
         ...
         getLValName("fields");
         for(x=0;x<fieldCtr;x++){
            output.write("public void setDC" + getLValName("fields") +"(String toSet){\n");
...
//               myProc = Runtime.getRuntime().exec("mkdir " + noExtension);
//               myProc = Runtime.getRuntime().exec("mv " + noExtension + ".java " + noExtension + "/"); 
//               myProc = Runtime.getRuntime().exec("javac " + noExtension + "/" + noExtension + ".java"); // compile the code
...
//move all generated actionlisteners (.java files) into the new (noExtension) directory as well
         ...
         try{ /* run the generated java code */
            ...
            myProc = Runtime.getRuntime().exec("java -cp " + noExtension + " " + noExtension);
...
//FIX COMPRUNOPT
         ...
         if(new File((noExtension + "/" + noExtension + ".class")).isFile()){ // check if .class file exists before running
