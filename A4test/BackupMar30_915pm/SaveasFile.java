...
public class SaveasFile extends JFrame implements ActionListener{
   ...
   public SaveasFile(String action){ /* action = the action to take after saving */
      theAction = action;
   ...
   public void actionPerformed(ActionEvent e){
      if(e.getSource()==saveasBtn){
         Dialogc.filename = saveasField.getText();
         Dialogc.updateStatusBar();
         try{
            fw = new FileWriter(Dialogc.filename);
            bw = new BufferedWriter(fw);
            bw.write((Dialogc.textA).getText());
            bw.close();
         }
         catch(Exception ex){
