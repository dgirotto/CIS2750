...
public class Dialogc extends JFrame implements ActionListener{
   ...
   boolean yaccCompiler=true;

   private JFrame selCompFrame;

   private String compilerOpts;
   private String runtimeOpts;
   private String javaCompiler="javac"; //default compiler
   private String runtimeCommand="java"; //default run command

   private String jCompPath;
   private String jvmPath;

   private String workingDir;
   private JRadioButton yaccButton;
   private JRadioButton jniButton;
   ...
   public void selCompiler(){
      selCompFrame = new JFrame("Select your Compiler");
      selCompFrame.setLayout(new BorderLayout());
      yaccButton = new JRadioButton("Yacc_Compiler");

      yaccButton.addActionListener(this);
      yaccButton.setSelected(true);

      jniButton = new JRadioButton("JNI_Compiler");
      jniButton.addActionListener(this);

      ButtonGroup group = new ButtonGroup();
      group.add(yaccButton);
      group.add(jniButton);

      selCompFrame.add(yaccButton,BorderLayout.WEST);
      selCompFrame.add(jniButton,BorderLayout.EAST);

      selCompFrame.setSize(100,100);
      selCompFrame.setDefaultCloseOperation(1);
      selCompFrame.setVisible(true);
   }
   ...
   public void promptAboutMenu(){
      ...
      myFrame.add(infoArea);
      myFrame.setSize(200,200);
	  infoArea.setText("About Dialogc\n\nName:Daniel Girotto\nStudent ID: 0783831\nEmail:dgirotto@uoguelph.ca");
	  infoArea.append("Dialogc is a compiler/interpreter which receives a title, fields, and buttons from the user\n");
	  infoArea.append("and builds a Java GUI interface when the user chooses to compile and run (provided his/her\n");
	  infoArea.append("input was valid). From that point, the user can attach action listeners of their own\n");
	  infoArea.append("to perform tasks to their liking.\n");
	  infoArea.setEditable(false);
      myFrame.setDefaultCloseOperation(1);
      myFrame.setVisible(true);
   }
   ...
   public void promptSave(){
      saveFrame = new JFrame("Save your file?");
      ...
      saveFrame.add((new JLabel("File has been modified!\nWould you like to save your changes?")),BorderLayout.NORTH);
      saveFrame.add(saveFileBtn,BorderLayout.WEST);
      saveFrame.add(dontSaveBtn,BorderLayout.EAST);
      saveFrame.setSize(200,200);
      saveFrame.setDefaultCloseOperation(1);
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      /* check if the current file is modified before blindly compiling */
         if(modFlag==true && newFlag==true){ /* user is trying to compile a new file */
            promptFileName();
         }
         else if(modFlag==true && newFlag==false){
            //save file
         }
         if(yaccCompiler==true){

            try{
               /* compile the generated java code */
               System.out.println("executing ./yadc " + filename);
               myProc = Runtime.getRuntime().exec("./yadc " + filename);
               nameOfFile = filename.replaceFirst("[.][^.]+$", "");
               if(e.getSource()==cmpRunBtn || e.getSource()==compRunOpt){
                  System.out.println("executing javac " + this.nameOfFile + ".java");
                  myProc = Runtime.getRuntime().exec("javac " + this.nameOfFile + ".java");
                  System.out.println("executing java " + this.nameOfFile);
         ...
         else if(compileSource()==1){
            generateGuiCode();
            try{
               /* compile the generated java code */
               if(compilerOpts==null){compilerOpts="";}
               myProc = Runtime.getRuntime().exec(javaCompiler + " " + compilerOpts + " " + this.nameOfFile + ".java");
               if(e.getSource()==cmpRunBtn || e.getSource()==compRunOpt){
                  if(runtimeOpts==null){runtimeOpts="";}
      ...
      else if(e.getSource()==yaccButton){
         yaccCompiler=true;
         selCompFrame.dispose();
      }
      else if(e.getSource()==jniButton){
         yaccCompiler=false;        
      }
      ...
      else if(e.getSource()==compileMode){
         selCompiler();
      }
         ...
         runtimeCommand = jRTFld.getText();
      }
      else if(e.getSource()==jCompSetBtn){
         javaCompiler = jCompFld.getText();
      }
      else if(e.getSource()==rtOptsSendBtn){
         runtimeOpts = rtOptsFld.getText();
      }
      else if(e.getSource()==compOptsSendBtn){
         compilerOpts = compOptsFld.getText();
      }
      else if(e.getSource()==saveFileBtn){
         //write current JTextArea to a file
