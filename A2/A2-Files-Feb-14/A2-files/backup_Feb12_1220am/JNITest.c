...
JNIEXPORT jstring JNICALL Java_JNITest_getTitleName(JNIEnv *env, jobject thisObj, jstring titleName){
   char *cName;
   const char *cStr;

   cStr = (*env)->GetStringUTFChars(env,titleName,NULL);
   cName = malloc(sizeof(char)*strlen(cStr));
   strcpy(cName,cStr);

   (*env)->ReleaseStringUTFChars(env,titleName,cStr); /* releasing resources */
